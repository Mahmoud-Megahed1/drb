<?php
/**
 * Rounds Gate Login Page
 * Entry point for rounds scanner staff
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['username'])) {
    ...
    $deviceName = trim($_POST['device_name'] ?? 'rounds_tablet');
    ...
    if (!checkRateLimit('login_' . $_SERVER['REMOTE_ADDR'], 5, 60)) {
        ...
        if ($result['success']) {
            if (hasPermission('rounds')) {
                ...
                logoutUser();
                $error = 'ليس لديك صلاحية الوصول لبوابة الجولات';
            }
        } else {
            $error = $result['error'];
        }
...
if (isLoggedIn() && hasPermission('rounds')) {
    header('Location: admin/rounds_scanner.php');
    exit;
}
?>
    ...
    <div class="login-box">
        ...
        <form method="POST">
            <div class="form-group">
                ...
                <input type="text" name="username" placeholder="rounds_user" required autofocus>
            </div>
            ...
            <div class="form-group">
                ...
                <input type="text" name="device_name" placeholder="مثال: tablet_1">
            </div>
